# DocuZen — Stellenbosch University · Tygerberg Medical Campus

Upload a document. Ask it a question. Get an answer with the exact page it came from.
...
Medical students at Tygerberg campus deal with an overwhelming volume of PDF material — lecture notes, clinical guidelines, textbook chapters, research papers, and past exam documents. Finding one specific piece of information often means scrolling through 80 pages or re-reading an entire chapter.

DocuZen fixes that. Upload any document up to 50MB, ask a question in plain English, and get a direct answer with the page number it came from. No internet search. No hallucinations from a general model. Just your document, answered.
...
3. Get a grounded answer and the exact page numbers the answer came from
4. Verify in seconds — the page citation is a first-class part of every response

Rate limiting is set to 60 requests per hour per IP. File uploads are validated by type and capped at 50MB.

---

## How it works under the hood

1. The document is parsed and split into overlapping text chunks preserving page boundaries
...
This is where RAG stopped being something I had read about and became something I actually understood. Figuring out the right chunk size, overlap strategy, and how to surface page citations forced me to think carefully about every step in the pipeline.
...
The Next.js frontend on Vercel was blocked by the same-origin policy. Added `CORSMiddleware` with explicit allowed origins read from the environment so local dev and production both work without opening CORS to `*`.

---

## Technical notes

- Async SQLAlchemy with `create_async_engine`, `AsyncSession`, and `async_sessionmaker` — all database calls are non-blocking
- Overlapping chunks preserve context at boundaries; each chunk stores its page number for citation surfacing
- `Chunk.embedding.cosine_distance(query_vector)` runs the similarity search inside PostgreSQL — no Python-side sorting needed
